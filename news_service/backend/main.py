...
API_DESCRIPTION = """
Internal News Service for stock_tracker.
...
is written through stock-service HTTP (news-service does not write quotes).
Summaries go through ``clients.llm_provider_client`` (LiteLLM), not chat-service.

## Auth
News endpoints require header:
...
2. **Search** — `POST /api/v1/news/search-and-summarize`: rank stored article
   ...
   LLM analysis
3. **Jobs** — `POST /jobs/news-update`: for every stock, fetch news -> summarize via LiteLLM -> HTTP update `stock_summery`

...
from constant import INTERNAL_API_KEY, INTERNAL_API_KEY_HEADER, NEWS_SERVICE_URL


class NewsServiceClient:
    """HTTP client for the internal news service."""
    ...
    def __init__(
        ...
        self._base_url = (base_url or NEWS_SERVICE_URL).rstrip("/")
    ...
    def _raise_from_response(self, response: httpx.Response) -> None:
        ...
        try:
            ...
            detail = response.text or "News Service request failed"
    ...
    async def search_and_summarize(self, symbol: str, query: str) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise HTTPException(502, "News Service returned an invalid response")
        summary = str(payload.get("summary") or "").strip()
        articles = payload.get("articles")
        has_articles = isinstance(articles, list) and bool(articles)
        if not summary and not has_articles:
            raise HTTPException(502, "News Service returned an empty response")
        return payload


news_service_client = NewsServiceClient()

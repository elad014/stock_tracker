...
from clients.news_service_client import news_service_client as news_service
...
CHAT_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Ask news-service about stored news_articles.text for a ticker. "
                "News-service searches and ranks article bodies, returns matching "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Get daily OHLCV history for a ticker from stock-service "
...
class ChatTools:
    ...
    async def ask_user_document(self, document_id: str, query: str) -> str:
        ...
        try:
            payload = await doc_service.ask_document(
                self._user_id,
                relative or None,
                question,
            )
    ...
    async def get_stock_news_summary(self, symbol: str, query: str) -> str:
        ...
        try:
            payload: dict[str, Any] = await news_service.search_and_summarize(
                ticker,
                question,
            )
        except Exception as exc:
    ...
    async def get_stock_price(self, symbol: str) -> str:
        ...
        if not ticker:
            return "symbol is required."
        try:
            payload = await stock_service.get_stock_by_symbol(ticker)
        except Exception as exc:
    ...
    async def get_stock_history(self, symbol: str, range_key: str) -> str:
        ...
        try:
            quote: Optional[dict[str, Any]] = await stock_service.get_stock_by_symbol(
                ticker
            )
            if quote is None:
                return f"Symbol not found: {ticker}."
            ...
            if not stock_id:
                return f"Symbol not found: {ticker}."
            bars: Any = await stock_service.get_stock_history(stock_id, window)
        except Exception as exc:
